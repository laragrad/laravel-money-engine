...
class EntryService
{
    ...
    public function executeInverseEntry(Operation $operation, Entry $entry)
    {
        ...
        $compensationEntry->sys_type_code = Entry::ENTRY_SYS_TYPE_INVERSE;
        ...
        $entry->compensation_kind_code = Entry::ENTRY_COMPENSATION_KIND_INVERSE;
        $entry->save();
     ...
     * @param Entry $entry
     */
    public function deleteNormalEntry(Entry $entry)
    {
        $entry->debitEntity->changeAccountSum($entry->db_account_code, -$entry->db_sum);
        $entry->debitEntity->save();
